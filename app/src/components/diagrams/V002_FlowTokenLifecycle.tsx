...
// C-shape layout: ISSUANCE(top-left) → CIRCULATION(top-right) → SOURCE REVIEW(bottom-right) → COMMONS(bottom-left)
...
const NODES: DiagramNode[] = [
  ...
  { id: 'circulation', label: 'Circulation',  definition: 'Flow moves through wages, contracts, commerce, and savings. Ordinary balances are not routinely decayed; fiscal pressure is handled through source-specific Commons Return.', docLink: 'ANNEX_X.md', accent: THEME.flow.accent,  accentBg: THEME.flow.accentBg },
  { id: 'sourceReview', label: 'Source Review', definition: 'Commons Return reviews public-created value, scarcity rents, resource use, platform rents, and large succession transfers without treating ordinary savings as the funding base.', docLink: 'ANNEX_D.md', accent: THEME.voice.accent, accentBg: THEME.voice.accentBg },
  { id: 'commons',     label: 'Commons Rail', definition: 'Commons Return receipts enter the lockbox for Essential Access, public rails, restoration, resilience reserves, and Universal Stake after fiscal gates are met.', docLink: 'ANNEX_D.md',  accent: THEME.ea.accent,    accentBg: THEME.ea.accentBg },
]
...
const BOXES = {
  ...
  sourceReview:{ x: 430, y: 160, w: 165, h: 60 },
  commons:     { x: 30,  y: 160, w: 145, h: 60 },
}

// Particle path segments (from box edge to box edge)
// p1: ISSUANCE right → CIRCULATION left  (top, blue)
// p2: CIRCULATION bottom → SOURCE REVIEW top  (right, gold)
// p3: SOURCE REVIEW left → COMMONS right      (bottom, green)
  ...
  return (
    <DiagramShell figId="V-002" title="Flow Token Lifecycle" nodes={NODES} activeNodeId={activeNodeId} onInternalLink={onInternalLink}>
      <svg viewBox="0 0 660 250" className="w-full" style={{ height: 250 }}>
        ...
        {box('sourceReview','SOURCE',      'review · Annex D', THEME.voice.accent, THEME.voice.fill)}
        {box('commons',     'COMMONS RAIL','EA · rails · stake', THEME.ea.accent,   THEME.ea.fill)}
