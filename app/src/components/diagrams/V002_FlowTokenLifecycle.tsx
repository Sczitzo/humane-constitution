...
// C-shape layout: ISSUANCE(top-left) -> CIRCULATION(top-right) -> SETTLEMENT(bottom-right) -> PUBLIC RAILS(bottom-left)
...
const NODES: DiagramNode[] = [
  ...
  { id: 'circulation', label: 'Circulation',  definition: 'Flow moves through wages, contracts, commerce, ordinary savings, and investment. It is the market instrument, not the survival floor and not a civic-power token.', docLink: 'ANNEX_X.md', accent: THEME.flow.accent,  accentBg: THEME.flow.accentBg },
  { id: 'settlement',  label: 'Settlement',   definition: 'Payments settle through public banking and audit rails. Stale, mistaken, or captured issuance is corrected through published review and unwind procedures rather than routine balance decay.', docLink: 'ANNEX_X.md', accent: THEME.voice.accent, accentBg: THEME.voice.accentBg },
  { id: 'rails',       label: 'Public Rails', definition: 'The payment, custody, oracle, ombuds, and essential-access support rails are funded through public finance and Commons Return controls, not private extraction from ordinary balances.', docLink: 'ANNEX_D.md',  accent: THEME.ea.accent,    accentBg: THEME.ea.accentBg },
]
...
const BOXES = {
  ...
  settlement:  { x: 430, y: 160, w: 165, h: 60 },
  rails:       { x: 30,  y: 160, w: 145, h: 60 },
}

// Particle path segments (from box edge to box edge)
// p1: ISSUANCE right → CIRCULATION left  (top, blue)
// p2: CIRCULATION bottom -> SETTLEMENT top  (right, gold)
// p3: SETTLEMENT left -> PUBLIC RAILS right (bottom, green)
  ...
  return (
    <DiagramShell figId="V-002" title="Flow Token Lifecycle" nodes={NODES} activeNodeId={activeNodeId} onInternalLink={onInternalLink}>
      <svg viewBox="0 0 660 250" className="w-full" style={{ height: 250 }}>
        ...
        <text x={538} y={132} textAnchor="middle" fontSize={8.5} fill={THEME.dim} fontFamily="monospace" transform="rotate(90,538,132)">settlement + audit</text>
        <text x={302} y={208} textAnchor="middle" fontSize={8.5} fill={THEME.dim} fontFamily="monospace">public rail support</text>
        ...
        {box('settlement',  'SETTLEMENT',  'public payment rail', THEME.voice.accent, THEME.voice.fill)}
        {box('rails',       'PUBLIC RAILS','EA · oracle · ombuds', THEME.ea.accent,   THEME.ea.fill)}

        {/* Footer note */}
        <text x={330} y={240} textAnchor="middle" fontSize={8.5} fill={THEME.dim} fontFamily="monospace" letterSpacing="0.04em">
          Flow stays in the market lane; ordinary balances are protected from routine decay
        </text>
      </svg>
    </DiagramShell>
  )
