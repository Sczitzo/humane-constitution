  ...
  return (
    <DiagramShell figId="V-003" title="Scarcity Escalation Ladder" nodes={NODES} activeNodeId={activeNodeId} onInternalLink={onInternalLink}>
      <svg viewBox="0 0 800 180" className="w-full" style={{ height: 180 }}>
        {ids.map((id, i) => {
          const x = 20 + i * 156, y = 130 - i * 22, isActive = activeNodeId === id
          return (
            <g key={id} opacity={0} style={{ cursor: 'pointer', filter: isActive ? `drop-shadow(0 0 6px ${colors[i]})` : undefined }} onClick={() => handleNodeClick(id)}>
              ...
              {isActive && <rect x={x} y={y} width={140} height={52} rx={6} fill="none" stroke={colors[i]} strokeWidth={1.5} opacity={0.5}><animate attributeName="opacity" values="0.5;0;0.5" dur="1.8s" repeatCount="indefinite" /></rect>}
              ...
              {i < 4 && <line x1={x + 140} y1={y + 26} x2={x + 156} y2={y + 4} stroke={colors[i]} strokeWidth={1} strokeDasharray="3,2" opacity={0.4} />}
            </g>
          )
        })}
