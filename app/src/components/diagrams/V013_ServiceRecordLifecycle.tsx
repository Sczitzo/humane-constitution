  ...
  return (
    <DiagramShell figId="V-013" title="Service Record — Civic Eligibility Lifecycle" nodes={NODES} activeNodeId={activeNodeId} onInternalLink={onInternalLink}>
      <svg viewBox="0 0 720 140" className="w-full" style={{ height: 140 }}>
        ...
        {STAGES.map((s, i) => {
          const x = startX + i * (boxW + gap), isActive = activeNodeId === s.id
          return (
            <g key={s.id} opacity={0} style={{ cursor: 'pointer', filter: isActive ? `drop-shadow(0 0 6px ${s.stroke})` : undefined }} onClick={() => handleNodeClick(s.id)}>
              ...
              {isActive && <rect x={x} y={cy} width={boxW} height={boxH} rx={6} fill="none" stroke={s.stroke} strokeWidth={1.5} opacity={0.5}><animate attributeName="opacity" values="0.5;0;0.5" dur="1.8s" repeatCount="indefinite" /></rect>}
