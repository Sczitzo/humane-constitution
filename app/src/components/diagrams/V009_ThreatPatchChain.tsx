        ...
        {cols.map((c, i) => {
          ...
          return (
            <g key={c.id} opacity={0} style={{ cursor: 'pointer', filter: isActive ? `drop-shadow(0 0 6px ${c.stroke})` : undefined }} onClick={() => handleNodeClick(c.id)}>
              ...
              {isActive && <rect x={c.x - 2} y={38} width={164} height={94} rx={7} fill="none" stroke={c.stroke} strokeWidth={1.5} opacity={0.5}><animate attributeName="opacity" values="0.5;0;0.5" dur="1.8s" repeatCount="indefinite" /></rect>}
              {i > 0 && <path d={`M${c.x - 40},85 L${c.x + 4},85`} fill="none" stroke={THEME.border} strokeWidth={1.5} markerEnd="url(#arrP)" />}
              <rect x={c.x} y={40} width={160} height={90} rx={8} fill={c.fill} stroke={c.stroke} strokeWidth={isActive ? THEME.strokeWidth.active : THEME.strokeWidth.normal} />
