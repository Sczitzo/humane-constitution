...
**Operating constraints:** Shared Storehouse does not replace Essential Access; it modifies the delivery mechanism for scarce categories. Allocation per identity or protected pseudonymous continuity path is bounded by oracle-confirmed supply. The overlay may change delivery method, substitution, queueing, or ration timing; it may not lower the CSM except as an honestly declared System Failure under Annex Y. It carries a mandatory sunset — automatically deactivating when supply restoration is confirmed by oracle consensus. Oracle failure during active Shared Storehouse triggers the P-022 fallback protocol: conservative hold → 48h REB window → 72h governance handoff.
...
Plain meaning: Shared Storehouse is the emergency rationing tool. It changes how scarce goods are delivered; it does not authorize the system to pretend the survival floor is lower, and it must turn off when the shortage is over.
