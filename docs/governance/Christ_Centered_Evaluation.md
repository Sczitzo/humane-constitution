...
## Session 27 Christ-Centered Evaluation — The Canonical-Home (Anti-Drift) Discipline (P-077, Acceptance_Protocol)

**What this session reviews.** A constitution change to `docs/constitution/Acceptance_Protocol.md`: one **canonical-home discipline** added to the P-073 Framework-First Intake gate, replacing four near-identical drift rules (status spine, appeal spine, evidence template, data doctrine) that the consolidation program itself had written four times. The four local rules shrink to a home-declaration plus their own enforcement plus a citation; the general principle now has one definition site. Reviewed by four parallel single-role panelists.

**Christ-centered alignment.** This is the program repenting of its own accretion (Matt 7:3–5 — removing the beam from one's own eye): the framework-first gate exists to stop duplication, and it had produced duplication; this patch holds it to its own rule. The substantive heart of the change is a mercy clause made general — *a domain may always exceed the floor its home sets; it may never fall below it* — which is the corpus's standing preference for the more-protected person (Matt 25:40) lifted from the data domain, where it was load-bearing, into a corpus-wide principle. The exception "runs only one way": protection may always be stronger, never weaker. That is the right asymmetry — the same asymmetry as a floor that can be exceeded but never breached.

**Babel-risk warning.** Centralizing a principle in the constitution makes one paragraph a single amendment point — the classic concentration risk. It is bounded honestly in the patch record: each home keeps its own named enforcement (the build-fail, the gates-nothing hook, the spine's procedural supremacy), so a captured central paragraph cannot silently switch off a home's teeth; and the gate's own corrigibility clause subjects the discipline to the same evidence test as everything else. The deeper anti-Babel note: the discipline forbids a domain from *declaring itself a new home* for a function already served — it refuses the proliferation of rival authorities, which is the orderly opposite of Babel's scattered tongues, achieved by subordination to a named common rule rather than by coerced uniformity (the uniform-procedure carve-out is narrow and named, not totalizing).

**Human dignity test.** Neutral-to-positive. No person's path changes; the stricter-text exception explicitly preserves every domain's stronger protection of a person's data, and explicitly refuses to let "uniformity" or "consolidation" strip a person of a protection they relied on. The legibility gain is real: a reader tracing why a domain defers to a spine now finds one stated principle instead of four subtly different restatements.

**Revision proposal.** The panel's incorporated fixes are recorded with the patch. Two were load-bearing and both improved honesty: the minimalist caught that the first draft left the four leaves *restating* the principle rather than pointing to it — making the true count 5 definition sites, not 1 — and the leaves were reduced to pure pointers so the consolidation claim became true rather than aspirational; the adversarial added a burden-and-appealability clause to the stricter-text exception, so "more protective" is no longer a self-certified shield but a falsifiable, recorded, appealable claim. The second is the more significant: the consolidated exception is now *stronger* than the four scattered versions it replaced, three of which carried no burden at all.

**Fruit test.** Expected good fruit: one place to read, amend, and audit the anti-drift principle; future spines inherit it by citation rather than re-inventing it; the program models the humility it preaches. Bad fruit to watch for: the central paragraph drifting from a home's actual enforcement over time (guarded by each home naming its own mechanism), and the temptation to treat "canonical home" as a power to be held rather than a rule to be served — watched by the same simplicity presumption that says the burden of proof is always on addition.

**Net assessment.** A faithful self-correction: the consolidation program turning its discipline on its own largest leftover restatement. **Scores 9 / 10**; the residual is that the discipline is review-enforced, not script-enforced, so a harmless non-diverging restatement can still accrete uncaught — `Active — unproven` in operation like the rest.
