...
| ANNEX_AZ §AZ2 — Tier 0 token mechanism specification | **OPEN** | The pseudonymous token mechanism for Tier 0 survival access is forward-referenced from ANNEX_AK §AK8.1 as a pre-operational prerequisite under INV-LAUNCH-1. Without a tamper-resistant token spec, the open-floor model cannot prevent session-linkage reconstruction by issuing infrastructure. | Draft ANNEX_AZ §AZ2 specifying token generation, expiry, non-aggregation guarantees, and audit constraints before any Phase 3 pilot. |
...
| Oracle calibration at small population | **OPEN** | INV-005 requires Tier-3 physical sampling; Cohort 1 is ~100 people. What does physical sampling look like at this scale? Standard oracle independence assumptions may not hold. | Before Cohort 1 enrollment: define minimum oracle node count for populations under 500; test whether FC-030/032 pairwise independence thresholds are achievable at small scale. |
