...
- **Class H-1 - Tier 3 operational parameter change:** adjusts a bounded dial already authorized elsewhere in the protocol, such as thresholds, refresh cadences, reserve ratios, or workflow timings. A change qualifies as Tier 3 only if it cites the specific original authorizing instrument and clause that grants the dial and confirms the proposed value stays within that instrument's stated bound. A dial that exceeds its original bound, or that cannot cite the original bound, is not a Tier 3 change and defaults upward to the higher applicable tier.
...
- **Upstream-grant verification:** every H-1 filing cites the specific original authorizing instrument and clause that establishes the dial and its bound, and re-verifies at filing that the proposed value remains within that stated bound. A filing that exceeds or cannot cite its original bound is rejected as misclassified and defaults upward to the higher applicable tier.
...
- **Passage rule:** both (a) at least 60% of valid weighted Voice cast in the quarterly decision cycle and (b) support from a simple majority of participating regions. Any H-1 change touching Voice or Service Record rules, verification, caps, half-life, proxy rules, tally math, challenge rights, or amendment-repair mechanics also requires a concurrent legitimacy check showing majority support in a stratified one-person-one-vote sample or equivalent low-burden referendum instrument; material divergence between the Voice result and the legitimacy check triggers mandatory CRP review before certification. **Material divergence** means any of: (a) the two instruments reach opposite outcomes (one supports the change and the other rejects it); or (b) their approval margins differ by more than 10 percentage points; or (c) the one-person-one-vote legitimacy check falls below 50% support while the Voice tally passes. Either instrument falling below its required threshold independently blocks certification regardless of divergence.
...
- **Invariant conscience review:** at least once every five years an independent panel — drawn from the Constitutional Review Panel, the Ombuds Office, and external persons of conscience including representatives of affected and vulnerable communities — conducts a public review of the Tier 1 invariants against lived outcomes and ethical conscience, and publishes its findings with any dissents. This review is purely advisory: it holds no amending power and cannot alter, suspend, or reinterpret any invariant. Its strongest available action is to **recommend** that a Refounding Convention be called under this section; the ordinary convention-call thresholds above still govern whether any such convention occurs. The review exists so the invariant core remains answerable to conscience and correction without being made cheaply amendable.
...
- **No emergency measure** may be renewed more than twice for the same underlying incident without a public post-mortem and supermajority judicial approval. A third renewal requires the affirmative vote of at least four-fifths of the reviewing judicial or constitutional review panel, recorded with published reasons; absent that approval the measure expires on schedule and reverts to the last known valid rule.
