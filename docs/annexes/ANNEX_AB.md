...
- **Essential goods and services** accessible through Essential Access shall be Essential Access-only in the baseline lane. Flow does not directly purchase the constitutional survival floor.
- **Essential Access execution** shall be non-delegable by default except for narrowly scoped, logged, and reversible caregiver or dependent pathways.
- **Essential Access balances** remain time-bounded and non-accumulative beyond the published continuity window to reduce stockpiling and arbitrage inventory.
...
*Capacity certifier — named and oracle-independent: The supply and throughput figures on which the protected definitions of "Scarcity" and "Capacity" (§AB5) rest are themselves an adversarial surface. Whoever certifies those figures sets the trigger for rationing the survival floor, and a captured certifier can move the boundary-stone through the number without ever touching the word. Accordingly, the body that certifies the capacity and throughput figures underlying "Scarcity" and "Capacity" shall be named and shall satisfy the oracle-independence standard governing external data sources (ANNEX_AL, ANNEX_M): it must be independent of any party that benefits from a scarcity declaration, and it may not be the same body that declares scarcity or allocates the rationed good. Every capacity and throughput certification must be auditable and independently checkable against published source data and methodology. A scarcity declaration resting on an uncertified throughput figure, on a figure certified by a body that does not satisfy the oracle-independence standard, or on a figure that cannot be independently challenged is invalid and confers no authority to ration the survival floor.*
...
- **Core terms** shall be classified by protection level. Constitutional terms such as survival, personhood, survival floor, non-convertibility, essential category, capacity, coercion, and scarcity shall not drift through operational relabeling.
...
*Escalation gatekeeper — named, independent, and accountable: The prima facie threshold of interpretive ambiguity is itself an anti-capture chokepoint. If the body that decides which challenges clear the threshold can quietly decline to forward inconvenient ones, capture migrates from the vocabulary to the procedure and the anti-capture process captures itself. Accordingly, the administrator of the prima facie threshold shall be named, and for any given challenge it must be independent of the party whose definition is being challenged; where the Federated Ombuds is itself the party whose interpretation is challenged, the threshold determination passes to an alternate independent custodian. Every challenge declined at the threshold must be logged in the public definition registry with the challenge text, the identity of the deciding body, and the stated reasons for declining, so that silence cannot operate as a capture instrument. A pattern of declined challenges is itself a reviewable dispute that any person may escalate.*
...
*Bounding of "to the extent necessary": "To the extent necessary" is a deliberately narrow phrase and shall not itself become a capture lever inside the anti-drift clause. It means only so far as is required to prevent a contradiction with, or an erosion of, a protection in the governing corpus. It confers no general supremacy on Annex AB and may never be invoked to expand this annex's reach beyond the resolution of an identified conflict. Where no protection of the governing corpus would be contradicted or eroded, the earlier language stands and Annex AB does not displace it.*
