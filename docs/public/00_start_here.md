...
The claimed contribution is not any one of these instruments — each has precedent and each, alone, has known failure modes. What is new is the specific *conjunction*: deliberately non-convertible instruments — survival, market money, and civic voice walled apart — bound together with published, testable thresholds for whether the walls are actually holding. Those thresholds include leakage tolerances, Commons Return incidence tests, and fiscal gates that must show public burdens falling on shared-value source bases rather than ordinary households. The bet is that separation plus falsification conditions is more defensible than separation asserted as a principle. Whether the conjunction survives contact with real incentives is exactly what has not been shown.
