...
| **FC-050** | `FLOW_DEMURRAGE_RATE` *(retired)* | 2 | RETIRED — superseded by the Annex D net-worth model | — | The per-balance idle-Flow fee was removed corpus-wide: there is no separate fee on liquid balances (Annex D §D9). The carrying-cost rate is now the progressive λ-bracket schedule on net worth — see FC-053. ID retained as a tombstone for citation integrity | Annex D §D9, `docs/SPECIFICATIONS.md` |
| **FC-051** | `FLOW_DEMURRAGE_REVIEW_CORRIDOR` *(retired)* | 3 | RETIRED — superseded by the Annex D net-worth model | — | The per-balance rate corridor is obsolete; λ-bracket rates are Tier-3 calibrated against the Demurrage Evidence Test Package (Annex D §D3) — see FC-053. ID retained as a tombstone | Annex D §D3, `docs/SPECIFICATIONS.md` |
| **FC-052** | `DEMURRAGE_PARTICIPATION_FLOOR_S` | 2 | 18 months × regional median monthly consumption (bootstrap $50,000 until oracle readiness) — bind before Flow launch | net worth (S) | Net-worth threshold below which no demurrage accrues. Too low reaches ordinary households; too high lets concentrated wealth escape the brackets (Annex D §D2.1). Redefined from the retired per-balance `FLOW_IDLE_THRESHOLD_DAYS` | Annex D §D2.1, `docs/SPECIFICATIONS.md`, Parameter Calibration Register |
| **FC-053** | `DEMURRAGE_PROGRESSIVE_LAMBDA_BRACKETS` | 2 | Founding calibration λ₁=26%, λ₂=30%, λ₃=38%, λ₄=46% per year — Tier-3 rate calibration | annual decay rate by net-worth tier | The sole carrying-cost rate on net worth above S; replaces the retired per-balance FC-050. Too low lets hoarding persist; too high confiscates working savers near the floor (Annex D §D2.4, §D3, §D3.5). Redefined from the retired per-balance `FLOW_RETIREMENT_EPSILON` | Annex D §D2.4, §D3, `docs/SPECIFICATIONS.md`, Parameter Calibration Register |
| **FC-054** | `PFCR_DEMURRAGE_ROUTING_SHARE` | 2 | PROPOSED VALUE: 0.65 (65% to PFCR receipts, 35% permanent retirement) — bind before activation | fraction of net-worth demurrage revenue | Split of net-worth demurrage revenue between Essential Access commons / public-rail funding and permanent retirement (Annex D §D8). 65% routing provides stable public funding; 35% retirement maintains supply discipline. Basis corrected from per-balance "demurrage application" to net-worth demurrage revenue | Annex D §D8, Article V, `docs/SPECIFICATIONS.md`, Parameter Calibration Register |
