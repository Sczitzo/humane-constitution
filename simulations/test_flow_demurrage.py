...
from simulations.model_outline import CitizenAgent, FlowState, CONFIG
...
def test_flow_demurrage_increments_idle_days(agent, monkeypatch):
    ...
    assert agent.flow_state == FlowState.IDLE
    assert agent.flow_balance == 100.0 # Balance should not change before threshold

def test_flow_demurrage_resets_idle_days_when_active(agent, monkeypatch):
    """Test that days_idle resets to 0 when state is ACTIVE."""
    monkeypatch.setitem(CONFIG, "idle_threshold_days", 30)

    agent.flow_state = FlowState.ACTIVE
    agent.days_idle = 15
    agent.flow_balance = 100.0
    ...
    assert agent.flow_state == FlowState.ACTIVE
    assert agent.flow_balance == 100.0

def test_flow_demurrage_applies_decay_at_threshold(agent, monkeypatch):
    ...
    monkeypatch.setitem(CONFIG, "min_flow_balance", 0.01)
...
def test_flow_demurrage_retires_balance_below_minimum(agent, monkeypatch):
    """Test that balance goes to 0 and state to RETIRED when decayed below min_flow_balance."""
    monkeypatch.setitem(CONFIG, "idle_threshold_days", 30)
    ...
    agent.flow_state = FlowState.IDLE
    agent.days_idle = 30
    agent.flow_balance = 1.0005  # slightly above 1.0, decay will push it below 1.0
    ...
    assert agent.flow_state == FlowState.RETIRED
