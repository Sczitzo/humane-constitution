import pytest
import numpy as np
from simulations.model_outline import CitizenAgent, ProtocolModel, FlowState, CONFIG
...
def test_demurrage_retirement(agent):
    ...
    # e.g. min_flow_balance is 0.01
    r = CONFIG["demurrage_rate_monthly"] / 30
    # Make it exactly equal to min_flow_balance before decay, so after decay it's below
    agent.flow_balance = CONFIG["min_flow_balance"]
...
def test_demurrage_committed_state_increments_days_idle(agent):
    """
    P-023 (COMMITTED state discipline — P-028 fix): Flow in escrow accrues demurrage.
    ...
    Prior adversarial simulation notes documented the divergence:
    ...
    assert agent.flow_balance == 100.0  # no decay yet (below threshold)
    assert agent.flow_state == FlowState.COMMITTED  # escrow still active
...
def test_demurrage_settled_state_does_not_affect_days_idle(agent):
    ...
    """
    agent.flow_state = FlowState.SETTLED
    agent.days_idle = 10
